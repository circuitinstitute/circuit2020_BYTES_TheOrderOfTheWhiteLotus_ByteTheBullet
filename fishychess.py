...
class TroutBot(Player):
    ...
    def get_queen(self, move_actions: List[chess.Move], seconds_left: float) -> Optional[chess.Move]:
        ...
        if enemy_queen_square:
            enemy_queen_attacker = self.board.attackers(self.color, enemy_queen_square)
            if enemy_queen_attacker:
                attacker_square = enemy_queen_attackers.pop()
                return chess.move(attacker_square, enemy_queen_square)
        try:
            self.board.turn = self.color
            self.board.clear_stack()
            result = self.engine.play(self.board, chess.engine.Limit(time=.05))
            return result.move
        except chess.engine.EngineError:
             print('Stockfish Engine bad state at "{}"'.format(self.board.fen()))
